...
if (dropZone && input) {
  ...
  dropZone.addEventListener('drop', e => {
    ...
    const dt = new DataTransfer(); dt.items.add(file); input.files = dt.files; showPreview(file);
  });
...
// Mode toggling (classify vs detect)
const modeSel = document.getElementById('mode');
const modelWrap = document.getElementById('model-wrap');
const topK = document.getElementById('top_k');
const minProb = document.getElementById('min_prob');
function applyMode() {
  const isDetect = modeSel && modeSel.value === 'detect';
  if (modelWrap) modelWrap.style.display = isDetect ? 'none' : '';
  if (topK) topK.disabled = isDetect;
  if (minProb) minProb.disabled = isDetect;
}
modeSel && (modeSel.addEventListener('change', applyMode), applyMode());

// Animate bars, bind model info, tabs, modal
window.addEventListener('load', () => {
  document.querySelectorAll('.bars .bar span').forEach(span => { const w = span.style.width; span.style.width = '0%'; requestAnimationFrame(() => requestAnimationFrame(() => span.style.width = w)); });
  ...
  const setInfo = (key) => { const data = infoMap[key] || {}; infoRoot && infoRoot.querySelectorAll('[data-field]').forEach(el => { const field = el.getAttribute('data-field'); if (field && data[field] !== undefined) el.textContent = data[field]; }); };
  if (select) { setInfo(select.value); select.addEventListener('change', () => setInfo(select.value)); }
  document.querySelectorAll('.tab-buttons button').forEach(btn => { btn.addEventListener('click', () => { document.querySelectorAll('.tab-buttons button').forEach(b => b.classList.remove('active')); document.querySelectorAll('.tab-panel').forEach(p => p.classList.remove('active')); btn.classList.add('active'); const id = btn.getAttribute('data-tab'); const panel = document.getElementById(`panel-${id}`); panel && panel.classList.add('active'); }); });
